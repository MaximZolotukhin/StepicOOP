...
class Clock:
    ...
    @classmethod
    def __check_time(cls, tm):# Проверка на соответвия заданному условию
        if int(tm):
            if 0 <= tm < 100000:
                return True
            else:
    ...
    def set_time(self, tm): # Сеттер - используется для изменения приватных данных
        if Clock.__check_time(tm):
            self.__time = tm

    def get_time(self): # Геттер - используется для вывода приватных данных
        return self.__time
