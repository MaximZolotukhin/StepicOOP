...
class TextInput:
    ...
    def check_name(cls, name):
        if cls.MAX_LENGHT >= len(name) >= cls.MIN_LENGHT and all(map(lambda x: x in cls.CHARS_CORRECT, name)):
            # all(map(lambda x: x in cls.CHARS_CORRECT, name)) all() возвращает True если все true,
            # map выбирает каждый символ из name и с помощью lambda проверяет его в списке корректных символов.
            # for symbol in name:
            #     if symbol in cls.CHARS_CORRECT:
            #         continue
            #     else:
            #         raise ValueError("некорректное поле name")
            return name
...
login = FormLogin(TextInput("Лог&*ин"), PasswordInput("Пароль"))
html = login.render_template()
print(html)