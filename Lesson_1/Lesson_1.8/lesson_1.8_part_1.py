...
class Router:  # для описания работы роутеров в сети (в данной задаче полагается один роутер);
    ...
    def link(self, server): # для присоединения сервера server (объекта класса Server) к роутеру
        # (для простоты, каждый сервер соединен только с одним роутером)
        self.servers[server.ip] = server
        server.router = self # Ссылка на объект Роутер

    def unlink(self, server):
        s =  self.servers.pop(server.ip, False) # Удаляем сервер из словаря
        if s: # Если сервер удалили
            s.router = None # То роутеру присваиваемт значение None

    def send_data(self):
        for val in self.buffer:
            if val.ip in self.servers:
                self.servers[val.ip].buffer.append(val)
        self.buffer.clear()

class Server:  # для описания работы серверов в сети;
    _ip = 1

    def __new__(cls, *args, **kwargs): # Создание ip по умолчанию
        cls._ip = cls._ip + 1
        return super().__new__(Server)

    def __init__(self):
        self.buffer = []  # список принятых пакетов (объекты класса Data, изначально пустой);
        self.ip = Server._ip  # IP-адрес текущего сервера.
        self.router = None

    def send_data(self, data): # для отправки информационного пакета data (объекта класса Data)
# с указанным IP-адресом получателя (пакет отправляется роутеру и сохраняется в его буфере - локальном свойстве buffer)
        if self.router:
            self.router.buffer.append(data)

    def get_data(self): # возвращает список принятых пакетов (если ничего принято не было, то возвращается пустой список)
        # и очищает входной буфер
        copy_bufer = self.buffer[:]
        self.buffer.clear()
        return copy_bufer
